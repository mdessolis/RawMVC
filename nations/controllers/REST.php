...
trait REST {
  ...
  public function api(){ 
    ...
    if (in_array($method, ['GET', 'PUT', 'POST', 'DELETE', 'PATCH']) ){
      ...
      $this->sendResponse("Method not supported: ". $method, 405);
    }
  }
   ...
   * It needs a method getFileteredList to be defined in the model
   */
  ...
  private function api_DELETE(){
    $input = json_decode(file_get_contents("php://input"), true); // true => associative array
    $id = $input['id'] ?? 0;
    if ($this->model->load($id)) {
