    ...
    for(let row of data){
      tbody.innerHTML += `
        <tr>
          ...
          <td onclick="this.contentEditable=true" onfocusout="editField(this, ${row.country_id}, 'name')">${row.name}</td>
...
/**
 * editField checks if the tag is editable and updates the db
 * 
 * Receives a tag, an id of a db resource and the fieldName to be updated.
 *  
 */
function editField(td, id, fieldName)
{
  if (td.contentEditable=="true"){
    let data = {
      id: id,
      [fieldName]: td.innerText
    };
...
function edit(id){
  ...
  .then( (data) => {
    ...
    /* region_id is managed with a select tag so we need to activate with
       the selected attribute its corresponding option element
    */
